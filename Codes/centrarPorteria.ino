...
enum SubEstadoBarrido {
    CENTRAR_ANTES_DE_IZQ, 
    MOVER_IZQ,            
    CENTRAR_ANTES_DE_DER, 
    MOVER_DER             
};

SubEstadoBarrido subEstado = CENTRAR_ANTES_DE_IZQ;
...
void estadoBuscandoPelota()
{
    ...
    // Prioridad 1: Si ve la pelota, abandona el barrido
    if(leerPelota(xBall))
    {
    ...
    //LÓGICA DE BARRIDO CON REFERENCIA EN PORTERÍA 
    int xGoal;
    bool vePorteria = leerPorteria(xGoal);
    
    direccionLateral = 0.0; // Detenido por defecto si no ve la portería

    switch(subEstado) {
        case CENTRAR_ANTES_DE_IZQ:
        case CENTRAR_ANTES_DE_DER:
            if(vePorteria) {
                // MARGEN REDUCIDO: Obliga al robot a volver al centro real de la cancha
                if (xGoal >= 155 && xGoal <= 165) {
                    subEstado = (subEstado == CENTRAR_ANTES_DE_IZQ) ? MOVER_IZQ : MOVER_DER;
                    tiempoAcumulado = 0; 
                } else {
                    int offsetDerecha = (320 - xGoal);
                    int offsetIzquierda = xGoal;
                    
                    if(xGoal > CENTRO_X)
                        direccionLateral = -0.005 * offsetDerecha; // Mueve a la Derecha
                    else
                        direccionLateral = 0.005 * offsetIzquierda; // Mueve a la Izquierda
                        
                    direccionLateral = constrain(direccionLateral, -0.4, 0.4); 
                }
            }
            break;
        ...
        case MOVER_IZQ:
            tiempoAcumulado += dt;
            direccionLateral = 0.25; // Positivo es Izquierda
            if (tiempoAcumulado >= 2000) {
                subEstado = CENTRAR_ANTES_DE_DER;
            }
            break;
            
        case MOVER_DER:
            tiempoAcumulado += dt;
            direccionLateral = -0.25; // Negativo es Derecha
            if (tiempoAcumulado >= 2000) {
                subEstado = CENTRAR_ANTES_DE_IZQ;
            }
            break;
    }
    // ----------------------------------------------------

    // Si los sensores de piso detectan línea, tienen prioridad y sobrescriben direccionLateral/Frontal
...
void loop()
{
  //ACTUALIZACIÓN DE TIEMPO (dt)
    unsigned long tiempoActual = millis();
    dt = tiempoActual - ultimoTiempo;
