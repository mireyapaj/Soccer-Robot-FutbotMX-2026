...
//VARIABLES DE TIEMPO Y BARRIDO
unsigned long ultimoTiempo = 0;
unsigned long dt = 0;
unsigned long tiempoAcumulado = 0;
bool moviendoIzquierda = true;       // Inicia moviéndose a la izquierda
unsigned long tiempoObjetivo = 2000; // Primer movimiento de 2 segundos desde el centro
// --------------------------------------------
...
void estadoBuscandoPelota()
{
    ...
    // --- LÓGICA DE BARRIDO DE PORTERÍA ---
    tiempoAcumulado += dt; 

    if (tiempoAcumulado >= tiempoObjetivo)
    {
        tiempoAcumulado = 0; 
        moviendoIzquierda = !moviendoIzquierda; // Invertir dirección
        
        // Después del primer viaje (2s), los recorridos de lado a lado necesitan 4s
        tiempoObjetivo = 4000; 
    }

    // Asignar velocidad base según la dirección elegida
    // (Si se va a la derecha primero, invierte el 0.25 y el -0.25)
    direccionLateral = moviendoIzquierda ? 0.3 : -0.3;
    // -------------------------------------
